  ...
  const handleFormChange = (fieldTitle: string, value: any) => {
    setFormValues((prev: FormValues) => {
      ...
      if (formErrors[fieldTitle]) {
        setFormErrors((prev: { [key: string]: string | null }) => ({
          ...prev,
          [fieldTitle]: null,
        }));
      }
    ...
    form.fields.forEach((field) => {
      if (
        ...
        (formValues[field.title] === undefined ||
          ...
          (Array.isArray(formValues[field.title]) &&
            formValues[field.title].length === 0))
      ) {
        errors[field.title] =
          field.validationMessage ||
          `${field.label || field.title} is required.`;
        isValid = false;
      }
      ...
      if (field.validationRegex && formValues[field.title]) {
        ...
        if (!regex.test(valueToTest)) {
          errors[field.title] =
            field.validationMessage ||
            `Invalid format for ${field.label || field.title}.`;
          isValid = false;
        }
      }
    });
  ...
  return (
    <Fade in={shouldDisplayForm} timeout={800}>
      ...
      >
        ...
        >
          {leadFormHeader ? ` ${leadFormHeader}` : "Please complete the form."}
        </Typography>
        ...
        <Box component="form" noValidate autoComplete="off">
          ...
          <Box sx={{ mt: 3, display: "flex", justifyContent: "flex-end" }}>
            ...
            >
              {leadFormButtonText ? `${leadFormButtonText}` : "Submit"}
            </Button>
          </Box>
        </Box>
      </Paper>
